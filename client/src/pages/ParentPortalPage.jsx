...
export default function ParentPortalPage() {
  ...
  useEffect(() => {
    api
      ...
      .then((r) => {
        setChildren(r.data);
        if (r.data.length) setSelectedChild(r.data[0]);
      })
  ...
  useEffect(() => {
    ...
    api
      .get(`/parent/child/${id}/attendance?limit=10`)
  ...
  if (!children.length) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-purple-600/60 dark:text-purple-300/60">
          ...
          {children.map((child) => (
            ...
            >
              ...
              <div className="text-left">
                <div>{child.name}</div>
                <div
                  className={`text-xs ${selectedChild?.id === child.id ? 'text-purple-200' : 'text-purple-500 dark:text-purple-400'}`}
                >
      ...
      {selectedChild && (
        <>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-purple-200">
                  {selectedChild.student_id} • {selectedChild.grade_name} {selectedChild.section}
                </p>
                <span
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <StatCard
              label="Pending Work"
              value={assignments.filter((a) => a.submission_status !== 'graded').length}
              color="orange"
            />
            <StatCard
              label="Average GPA"
              value={
                grades.length
                  ? (grades.reduce((s, g) => s + (g.gpa || 0), 0) / grades.length).toFixed(1)
                  : '—'
              }
              color="purple"
            />
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6">
                ...
                {grades.length ? (
                  grades.slice(0, 5).map((g) => (
                    <div
                      key={g.course_id}
                      className="flex justify-between py-2 border-b border-purple-50 dark:border-gray-800 last:border-0"
                    >
              ...
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6">
                ...
                {summary.map((s) => (
                  <div
                    ...
                    className="flex justify-between py-2 border-b border-purple-50 dark:border-gray-800 last:border-0"
                  >
                    <span className="text-sm capitalize text-purple-800 dark:text-purple-200">
                      {s.status}
                    </span>
                    <span className="text-sm font-bold text-purple-600 dark:text-purple-400">
                      {s.count} days
                ...
                {!summary.length && (
                  <p className="text-sm text-purple-400">No attendance records</p>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === 'grades' && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden">
              <table className="w-full">
                ...
                <thead>
                  <tr className="bg-purple-50 dark:bg-gray-800">
                    ...
                    <th
                      scope="col"
                      className="px-4 py-3 text-center text-xs font-bold text-purple-600 dark:text-purple-400 uppercase"
                    >
                      Assignment
                    </th>
                    <th
                ...
                <tbody>
                  {grades.map((g) => (
                    <tr
                      key={g.course_id}
                      className="border-t border-purple-50 dark:border-gray-800"
                    >
                      <td className="px-4 py-3 text-sm font-medium text-purple-900 dark:text-purple-100">
                        {g.course_title}
                      </td>
                      <td className="px-4 py-3 text-sm text-center text-purple-700 dark:text-purple-300">
                        {g.assignment_score || '—'}
                      </td>
                      <td className="px-4 py-3 text-sm text-center text-purple-700 dark:text-purple-300">
                        {g.quiz_score || '—'}
                      ...
                      <td className="px-4 py-3 text-sm text-center font-bold text-purple-600 dark:text-purple-400">
                        {g.gpa || '—'}
                      </td>
                    </tr>
                  ))}
              ...
              {!grades.length && (
                <p className="p-6 text-center text-sm text-purple-400">No grades recorded yet</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'attendance' && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden">
                ...
                <tbody>
                  {(attendance?.records || []).map((r, i) => (
                    <tr key={i} className="border-t border-purple-50 dark:border-gray-800">
                      <td className="px-4 py-3 text-sm text-purple-900 dark:text-purple-100">
                        {r.date}
                      </td>
              ...
              {!attendance?.records?.length && (
                <p className="p-6 text-center text-sm text-purple-400">No attendance records</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'assignments' && (
            <div className="space-y-3">
              {assignments.map((a) => (
                <div
                  key={a.id}
                  className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-4 flex items-center justify-between"
                >
                  <div>
                    ...
                    <p className="text-xs text-purple-500 dark:text-purple-400">
                      {a.course_title} • Due: {a.due_date || 'No due date'}
                    </p>
                  </div>
              ...
              {!assignments.length && (
                <p className="text-center text-sm text-purple-400 py-8">No assignments</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'timetable' && (
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden">
                ...
                <tbody>
                  {timetable.map((t) => (
                    <tr key={t.id} className="border-t border-purple-50 dark:border-gray-800">
                      <td className="px-4 py-3 text-sm font-medium text-purple-900 dark:text-purple-100">
                        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][t.day_of_week]}
                      </td>
                      <td className="px-4 py-3 text-sm text-purple-700 dark:text-purple-300">
              ...
              {!timetable.length && (
                <p className="p-6 text-center text-sm text-purple-400">No timetable available</p>
              )}
            </div>
          )}
        </>
...
const StatusBadge = memo(function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${styles[status] || styles.submitted}`}
    >
      {status}
    </span>
  );
