...
export default function AcademicPage() {
  ...
  const openEditForm = (type, item) => {
    setFormType(type);
    setEditingTarget({ type, id: item.id });
    if (type === 'year') {
      setFormData({
        name: item.name,
  ...
  const handleSubmit = (e) => {
    ...
    const request = editingTarget
      ? api.put(`${basePath}/${editingTarget.id}`, formData)
      : api.post(basePath, formData);
    request
      .then(() => {
        setShowForm(false);
        setEditingTarget(null);
      ...
      .catch(() => setError(editingTarget ? t('academic.updateError') : t('academic.createError')))
      ...
      {isAdmin && (
        <div className="mb-6">
          <button
            onClick={() =>
              openForm(
                activeTab === 'years' ? 'year' : activeTab === 'semesters' ? 'semester' : 'holiday'
              )
            }
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold text-purple-900 dark:text-purple-100 mb-4">
              {editingTarget
                ? t(`academic.edit${formType.charAt(0).toUpperCase() + formType.slice(1)}`)
        ...
        <div className="space-y-4">
          {years.length > 0 ? (
            years.map((y) => (
              <div
                key={y.id}
                className="bg-white dark:bg-gray-900 rounded-2xl shadow-md shadow-purple-100/50 p-6 flex items-center justify-between"
              >
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isAdmin && (
                    <>
                      <button
                        onClick={() => openEditForm('year', y)}
                        className="p-2 text-purple-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/20 rounded-lg cursor-pointer transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
                          viewBox="0 0 24 24"
                      ...
                      <button
                        onClick={() => setDeleteTarget({ type: 'year', id: y.id })}
            ...
            semesters.map((s) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-purple-600 dark:text-purple-400">
                    {s.year_name} · {s.start_date} — {s.end_date}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {s.is_current && (
                  ...
                  {isAdmin && (
                    <>
                      <button
                        onClick={() => openEditForm('semester', s)}
                        className="p-2 text-purple-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/20 rounded-lg cursor-pointer transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
                      ...
                      <button
                        onClick={() => setDeleteTarget({ type: 'semester', id: s.id })}
                        className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg cursor-pointer transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
          ...
          {holidays.length > 0 ? (
            holidays.map((h) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium ${holidayTypeColors[h.type] || 'bg-gray-100 text-gray-700'}`}
                  >
                  ...
                  {isAdmin && (
                    <>
                      <button
                        onClick={() => openEditForm('holiday', h)}
                        className="p-2 text-purple-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/20 rounded-lg cursor-pointer transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
