...
export default function CertificatesPage() {
  ...
  const [form, setForm] = useState({
    student_id: '',
    type: 'completion',
    data: { description: '', academic_year: '2026-2027', grade: '', gpa: '' },
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-purple-600/60 dark:text-purple-300/60 mt-1">
            {isAdmin || isTeacher ? 'Generate and manage certificates' : 'View your certificates'}
          </p>
        </div>
        {(isAdmin || isTeacher) && (
          <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-48 bg-purple-200 dark:bg-gray-700 rounded-2xl" />
          ))}
        </div>
      ) : certificates.length ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificates.map((cert) => (
            <div
              key={cert.id}
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-shadow"
            >
              ...
              <div className="p-6">
                ...
                <div className="space-y-1 text-sm text-purple-700 dark:text-purple-300 mb-4">
                  <p>
                    <span className="text-purple-500 dark:text-purple-400">Serial:</span>{' '}
                    {cert.serial_number}
                  </p>
                  <p>
                    <span className="text-purple-500 dark:text-purple-400">Issued:</span>{' '}
                    {new Date(cert.issued_at).toLocaleDateString()}
                  </p>
                  <p>
                    <span className="text-purple-500 dark:text-purple-400">By:</span>{' '}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-12 text-center">
          ...
          <p className="text-sm text-purple-500 dark:text-purple-400">
            {isAdmin || isTeacher
              ? 'Generate your first certificate using the button above.'
            ...
            <form onSubmit={handleGenerate} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setForm({ ...form, data: { ...form.data, description: e.target.value } })
                  }
                  rows={3}
                  className="w-full px-3 py-2 bg-purple-50 dark:bg-gray-700 border border-purple-200 dark:border-gray-600 rounded-lg text-sm text-purple-900 dark:text-purple-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, data: { ...form.data, grade: e.target.value } })
                    }
                    className="w-full px-3 py-2 bg-purple-50 dark:bg-gray-700 border border-purple-200 dark:border-gray-600 rounded-lg text-sm text-purple-900 dark:text-purple-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    placeholder="e.g. A"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    value={form.data.gpa}
                    onChange={(e) =>
                      setForm({ ...form, data: { ...form.data, gpa: e.target.value } })
                    }
                    className="w-full px-3 py-2 bg-purple-50 dark:bg-gray-700 border border-purple-200 dark:border-gray-600 rounded-lg text-sm text-purple-900 dark:text-purple-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    placeholder="e.g. 3.8"
                  />
