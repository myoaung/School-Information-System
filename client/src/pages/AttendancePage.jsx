    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-8">
        <div className="w-10 h-10 bg-purple-100 rounded-xl flex items-center justify-center">
          <svg className="w-5 h-5 text-purple-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
        </div>
        <h1 className="text-3xl font-extrabold text-purple-900">{t('attendance.title')}</h1>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-md shadow-purple-100/50 p-6 mb-6">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium text-purple-700 mb-1">{t('attendance.selectClass')}</label>
            <select
              value={selectedClass}
          ...
          <div>
            <label className="block text-sm font-medium text-purple-700 mb-1">{t('attendance.date')}</label>
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className="px-4 py-2 border border-purple-200 rounded-xl text-sm focus:outline-none focus:border-purple-500"
            />
          </div>
        </div>
      </div>
      ...
      {message && (
        <div className={`px-4 py-3 rounded-xl mb-6 text-sm ${message.includes('Error') ? 'bg-red-50 border border-red-200 text-red-600' : 'bg-green-50 border border-green-200 text-green-600'}`}>
          {message}
        </div>
      )}
      ...
      {selectedClass && (
        loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-purple-50">
                  {attendance.map(a => (
                    <tr key={a.user_id} className="hover:bg-purple-50/50">
                      <td className="px-4 py-3 text-purple-900 font-medium">{a.name}</td>
                      <td className="px-4 py-3 text-purple-600">{a.student_id || '—'}</td>
                          ...
                          {['present', 'absent', 'late', 'leave'].map(status => (
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors cursor-pointer ${
                                a.status === status ? statusColors[status] : 'bg-white text-purple-400 border-purple-200 hover:bg-purple-50'
                              }`}
                            >
                              {t(`attendance.${status}`)}
                            </button>
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow-md shadow-purple-100/50">
            <p className="text-purple-500">{t('attendance.noStudents')}</p>
          </div>
        )
      )}
    </div>
