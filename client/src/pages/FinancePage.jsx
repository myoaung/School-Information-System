      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-purple-600/60 dark:text-purple-300/60 mt-1">
            {isAdmin ? 'Manage fees, invoices and payments' : 'View your fees and payments'}
          </p>
        </div>
        {isAdmin && (
          <button
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <StatCard
              ...
              value={formatCurrency(overview.totalInvoiced)}
              color="purple"
            />
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6">
            ...
            <div className="space-y-2">
              {overview.recentPayments.map((p) => (
                <div
                  key={p.id}
                  className="flex items-center justify-between p-3 bg-purple-50 dark:bg-gray-800 rounded-xl"
                >
                  <div>
                    ...
                    <p className="text-xs text-purple-500 dark:text-purple-400">
                      {p.payment_method} • {p.reference || 'No ref'}
                    </p>
                  </div>
              ...
              {!overview.recentPayments.length && (
                <p className="text-sm text-purple-400 text-center py-4">No payments yet</p>
              )}
            </div>
          </div>
        </>
          ...
          <div className="md:col-span-1 space-y-2 max-h-[600px] overflow-y-auto">
            {loading ? (
              <div className="animate-pulse space-y-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-20 bg-purple-200 dark:bg-gray-700 rounded-xl" />
                ))}
              </div>
            ) : (
              invoices.map((inv) => (
            ...
            {!loading && !invoices.length && (
              <p className="text-sm text-purple-400 text-center py-8">No invoices</p>
            )}
          </div>
          ...
          <div className="md:col-span-2">
            {invoiceDetail ? (
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    <h3 className="text-xl font-bold text-purple-900 dark:text-purple-100">
                      Invoice INV-#{invoiceDetail.id}
                    </h3>
                    <p className="text-sm text-purple-500 dark:text-purple-400">
                      {invoiceDetail.student_name} • {invoiceDetail.student_code}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="p-3 bg-purple-50 dark:bg-gray-800 rounded-xl">
                    ...
                    <p className="text-lg font-bold text-green-600 dark:text-green-400">
                      {formatCurrency(
                        invoiceDetail.payments?.reduce((s, p) => s + p.amount, 0) || 0
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {invoiceDetail.payments?.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950/20 rounded-xl"
                    >
                      <div>
                        <p className="text-sm font-medium text-purple-900 dark:text-purple-100">
                        ...
                        <p className="text-xs text-purple-500">
                          {p.reference || 'No reference'} • {new Date(p.paid_at).toLocaleString()}
                        </p>
                      </div>
                  ...
                  {!invoiceDetail.payments?.length && (
                    <p className="text-sm text-purple-400 text-center py-4">No payments recorded</p>
                  )}
                </div>
              </div>
            ) : (
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-12 text-center">
                <svg
                  className="w-16 h-16 text-purple-200 dark:text-purple-800 mx-auto mb-4"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1"
      ...
      {tab === 'fees' && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden">
          <table className="w-full">
            ...
            <thead>
              <tr className="bg-purple-50 dark:bg-gray-800">
                <th
                  scope="col"
                  className="px-4 py-3 text-left text-xs font-bold text-purple-600 dark:text-purple-400 uppercase"
                >
                  Fee Type
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-4 py-3 text-right text-xs font-bold text-purple-600 dark:text-purple-400 uppercase"
                >
                  Amount
                </th>
                <th
            ...
            <tbody>
              {fees.map((f) => (
                <tr key={f.id} className="border-t border-purple-50 dark:border-gray-800">
                  <td className="px-4 py-3 text-sm font-medium text-purple-900 dark:text-purple-100">
                    {f.fee_type}
                  </td>
                  <td className="px-4 py-3 text-sm text-purple-700 dark:text-purple-300">
                    {f.grade_name || 'All Grades'}
                  </td>
                  <td className="px-4 py-3 text-sm text-right font-bold text-purple-700 dark:text-purple-300">
          ...
          {!fees.length && (
            <p className="p-6 text-center text-sm text-purple-400">No fee structures defined</p>
          )}
        </div>
      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-purple-100 dark:border-gray-700">
              ...
              <button
                ...
                className="p-1 hover:bg-purple-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer"
              >
                <svg
            ...
            <form onSubmit={handleCreateInvoice} className="p-6 space-y-4">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setForm({
                      ...
                      amount: fee ? String(fee.amount) : form.amount,
                    });
                  }}
                  className="w-full px-3 py-2 bg-purple-50 dark:bg-gray-700 border border-purple-200 dark:border-gray-600 rounded-lg text-sm text-purple-900 dark:text-purple-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                >
                  ...
                  {fees.map((f) => (
                    <option key={f.id} value={f.id}>
                      {f.fee_type} - {f.grade_name || 'All'} ({formatCurrency(f.amount)})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-lg cursor-pointer"
                >
                  Create
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handlePayment} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg cursor-pointer"
                >
                  Record
                </button>
              </div>
...
const StatusBadge = memo(function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${styles[status] || styles.pending}`}
    >
      {status}
    </span>
  );
