...
export default function AnalyticsPage() {
  ...
  if (!isAdmin && !isTeacher) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-purple-500 mt-2">Analytics are available for teachers and administrators.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-purple-900 dark:text-purple-100">Predictive Analytics</h1>
        <p className="text-purple-600/60 dark:text-purple-300/60 mt-1">AI-powered student risk assessment and early warning system</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard label="Average Risk Score" value={stats.avgRiskScore?.toFixed(1) || '—'} color="purple" />
          <StatCard label="Risk Rate" value={`${stats.riskPercentage?.toFixed(1) || 0}%`} color="orange" />
        </div>
      )}
            ...
            {['critical', 'high', 'medium', 'low'].map(level => {
              ...
              return (
                <div key={level} className="text-center">
                  <div className={`w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-2xl font-bold text-white mb-2 ${
                    level === 'critical' ? 'bg-red-500' : level === 'high' ? 'bg-orange-500' : level === 'medium' ? 'bg-yellow-500' : 'bg-green-500'
                  }`}>{count}</div>
                  <p className="text-sm font-medium text-purple-900 dark:text-purple-100 capitalize">{level}</p>
      ...
      {alerts.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-purple-900 dark:text-purple-100">Active Alerts ({alerts.length})</h3>
          </div>
          <div className="space-y-2">
            {alerts.slice(0, 5).map((alert, i) => (
              <div key={i} className={`p-3 rounded-xl border-l-4 ${
                ...
                alert.severity === 'high' ? 'bg-orange-50 dark:bg-orange-950/20 border-orange-500' :
                'bg-yellow-50 dark:bg-yellow-950/20 border-yellow-500'
              }`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-purple-900 dark:text-purple-100">{alert.studentName}</p>
                    <p className="text-xs text-purple-500 dark:text-purple-400">{alert.message}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${
                    alert.severity === 'critical' ? 'bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300' :
                    alert.severity === 'high' ? 'bg-orange-100 text-orange-700 dark:bg-orange-950/40 dark:text-orange-300' :
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={filter.minScore} onChange={e => setFilter({...filter, minScore: parseInt(e.target.value)})}
      ...
      <div className="space-y-3">
        {atRisk.map(student => (
          <div key={student.userId} className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-5 hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  <p className="text-sm text-purple-500 dark:text-purple-400">
                    ...
                    <span className={`ml-1 capitalize font-medium ${
                      student.riskLevel === 'critical' ? 'text-red-600 dark:text-red-400' :
                      student.riskLevel === 'high' ? 'text-orange-600 dark:text-orange-400' :
                      student.riskLevel === 'medium' ? 'text-yellow-600 dark:text-yellow-400' :
              ...
              <div className="flex items-center gap-3">
                <div className="hidden md:flex gap-2">
                  {student.factors?.map((f, i) => (
                    <span key={i} className="text-xs px-2 py-1 bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded-full">
                      {f.type}: {f.value}
                    </span>
                  ))}
                </div>
        ...
        {!atRisk.length && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-8 text-center">
            <div className="w-16 h-16 bg-green-100 dark:bg-green-950/40 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            </div>
            <h3 className="text-lg font-bold text-purple-900 dark:text-purple-100">No At-Risk Students</h3>
            <p className="text-sm text-purple-500 mt-1">All students are performing well above the risk threshold.</p>
          </div>
        )}
      </div>
    </div>
...
function LoadingSkeleton() {
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-pulse">
      ...
      {[1,2,3].map(i => <div key={i} className="h-20 bg-purple-200 dark:bg-gray-700 rounded-2xl mb-3" />)}
    </div>
  );
}
