  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-3xl font-extrabold text-purple-900">{t('assignments.title')}</h1>
      </div>
      ...
      {assignments.length > 0 ? (
        <div className="space-y-4">
          {assignments.map(a => (
            <div key={a.id} className="bg-white rounded-2xl shadow-md shadow-purple-100/50 p-6">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-purple-900">{a.title}</h3>
                  <p className="text-sm text-purple-600/60 mt-1">{a.course_title}</p>
                  {a.description && <p className="text-sm text-purple-700 mt-2">{a.description}</p>}
                  <div className="flex flex-wrap gap-4 mt-3 text-xs text-purple-500">
              ...
              {submitting === a.id && (
                <div className="mt-4 pt-4 border-t border-purple-100">
                  <textarea value={submitContent} onChange={e => setSubmitContent(e.target.value)}
                    ...
                    className="w-full px-4 py-2 border border-purple-200 rounded-xl text-sm focus:outline-none focus:border-purple-500 resize-none" rows={3} />
              ...
              {(isAdmin || isTeacher) && a.submissions?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-purple-100">
                  <h4 className="text-sm font-bold text-purple-700 mb-3">{t('assignments.submissions')} ({a.submissions.length})</h4>
                  <div className="space-y-2">
                    {a.submissions.map(s => (
                      <div key={s.id} className="flex flex-col sm:flex-row sm:items-center gap-2 bg-purple-50 rounded-xl p-3">
                        <div className="flex-1">
                          <span className="text-sm font-medium text-purple-900">{s.student_name}</span>
                          <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${
                            s.status === 'graded' ? 'bg-green-100 text-green-700' : s.status === 'late' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'
                          }`}>{s.status}</span>
                          {s.score != null && <span className="ml-2 text-xs text-purple-600">{s.score}/{a.max_score}</span>}
                        </div>
                        {s.status !== 'graded' && (
                        ...
                        {grading === s.id && (
                          <div className="flex gap-2 items-center mt-2">
                            <input type="number" value={gradeScore} onChange={e => setGradeScore(e.target.value)} placeholder={t('assignments.score')}
                              className="w-20 px-2 py-1 border border-purple-200 rounded-lg text-sm" />
                            <input type="text" value={gradeFeedback} onChange={e => setGradeFeedback(e.target.value)} placeholder={t('assignments.feedback')}
                              className="flex-1 px-2 py-1 border border-purple-200 rounded-lg text-sm" />
        ...
        <div className="text-center py-16 bg-white rounded-2xl shadow-md shadow-purple-100/50">
          ...
          <p className="text-purple-500">{t('assignments.noAssignments')}</p>
        </div>
      )}
    </div>
  );
