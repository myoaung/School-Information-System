...
router.post('/', authMiddleware, roleMiddleware('admin', 'teacher'), gradeRules, async (req, res) => {
  try {
    ...
    const existing = await db.get('SELECT id FROM gradebook WHERE student_id = ? AND course_id = ?', [student_id, course_id]);
    if (existing) {
      await db.run('UPDATE gradebook SET assignment_score = ?, quiz_score = ?, exam_score = ?, final_grade = ?, gpa = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [assignment_score || 0, quiz_score || 0, exam_score || 0, final_grade || null, gpa || null, existing.id]);
      res.json({ message: 'Grade updated' });
    } else {
      const result = await db.run('INSERT INTO gradebook (student_id, course_id, assignment_score, quiz_score, exam_score, final_grade, gpa) VALUES (?, ?, ?, ?, ?, ?, ?)', [student_id, course_id, assignment_score || 0, quiz_score || 0, exam_score || 0, final_grade || null, gpa || null]);
