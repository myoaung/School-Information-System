...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const quizzes = await db.all('SELECT * FROM quizzes WHERE course_id = ? ORDER BY due_date', [
      req.params.id,
    ]);
  ...
  async (req, res) => {
    try {
      ...
      if (!class_id || !subject_id || !title)
        return res.status(400).json({ error: 'class_id, subject_id, title required' });
      const result = await db.run(
        'INSERT INTO courses (class_id, subject_id, title, description) VALUES (?, ?, ?, ?)',
        [class_id, subject_id, title, description || null]
      );
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.run(
        ...
        [req.params.id, title, content || null, lesson_order || 0, duration_minutes || null]
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.run(
        'INSERT INTO resources (course_id, title, type, url, description, uploaded_by) VALUES (?, ?, ?, ?, ?, ?)',
        [req.params.id, title, type, url || null, description || null, req.user.id]
      );
