...
router.post(
  ...
  async (req, res) => {
    try {
      const { question_text, question_type, options, correct_answer, points, question_order } =
        req.body;
...
router.put('/:id', authMiddleware, requirePermission('courses', 'update'), async (req, res) => {
  try {
    ...
    if (!course_id || !title)
      return res.status(400).json({ error: 'course_id and title required' });
    await db.run(
      'UPDATE quizzes SET course_id = ?, title = ?, description = ?, time_limit_minutes = ?, max_score = ?, due_date = ? WHERE id = ?',
      [
        course_id,
        title,
        description || null,
...
router.post('/:id/attempt', authMiddleware, async (req, res) => {
  try {
    ...
    const questions = await db.all('SELECT * FROM quiz_questions WHERE quiz_id = ?', [
      req.params.id,
    ]);
    ...
    const result = await db.run(
      'INSERT INTO quiz_attempts (quiz_id, student_id, answers, score, completed_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)',
      [
        req.params.id,
        req.user.id,
