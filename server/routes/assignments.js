  ...
  async (req, res) => {
    try {
      ...
      const result = await db.run(
        'INSERT INTO assignments (course_id, title, description, due_date, max_score, allow_late, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [
          course_id,
          title,
          description || null,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await db.run(
        'UPDATE submissions SET score = ?, feedback = ?, status = ?, graded_at = CURRENT_TIMESTAMP, graded_by = ? WHERE id = ?',
        [score, feedback || null, 'graded', req.user.id, req.params.id]
      );
